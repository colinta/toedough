...
Node.prototype.insert = function(element, i) {
  ...
  (function emit(el) {
    var attaching = el.detached !== self.detached;
    el.detached = self.detached;
    if (attaching) el.emit('attach');
...
function ScrollableBox(options) {
  ...
  if (options.keys && !options.ignoreKeys) {
    this.on('keypress', function(ch, key) {
      ...
      else if (key.name === 'down' || options.vi && key.name === 'j') {
        self.scroll(1);
        self.screen.render();
      }
      else if (key.name = 'pageup' || options.vi && key.name === 'u' && key.ctrl) {
        self.scroll(-(self.height / 2 | 0) || -1);
        self.screen.render();
      }
      else if (key.name = 'pagedown' || options.vi && key.name === 'd' && key.ctrl) {
        self.scroll(self.height / 2 | 0 || 1);
        self.screen.render();
      }
      else if (options.vi && key.name === 'b' && key.ctrl) {
        self.scroll(-self.height || -1);
        self.screen.render();
      }
      else if (options.vi && key.name === 'f' && key.ctrl) {
        self.scroll(self.height || 1);
        self.screen.render();
      }
      else if (key.name = 'home' || options.vi && key.name === 'g' && !key.shift) {
        self.scroll(-self._clines.length);
        self.screen.render();
      }
      else if (key.name = 'end' || options.vi && key.name === 'g' && key.shift) {
        self.scroll(self._clines.length);
        self.screen.render();
      }
    });
...
ScrollableBox.prototype.scrollToTop = function() {
  this.scrollTo(0);
  this.screen.render();
}

ScrollableBox.prototype.scrollToBottom = function() {
  this.scrollTo(this._clines.length);
  this.screen.render();
}
...
List.prototype.create = function(item) { return this.createListItem(item); };
...
List.prototype.appendItem = function(item) {
  var node = this.createListItem(item);
...
List.prototype.appendNode = function(node) {
  ...
  Styles.forEach(function(name) {
    node.style[name] = function() {
      var attr = self.items[self.selected] === node
        ? self.style.selected[name]
        : self.style.item[name];
      if (typeof attr === 'function') attr = attr(node);
      return attr;
    };
  });
...
List.prototype.onKeypress = function(ch, key) {
  ...
  if (key.name === 'up' || (self.vi && key.name === 'k')) {
    self.up();
    self.screen.render();
    return;
  }
  if (key.name === 'down' || (self.vi && key.name === 'j')) {
    self.down();
    self.screen.render();
    return;
  }
  if (key.name === 'enter'
      || (self.vi && key.name === 'l' && !key.shift)) {
    self.enterSelected();
    return;
  }
  if (key.name === 'escape' || (self.vi && key.name === 'q')) {
    self.cancelSelected();
    return;
  }
  if (self.vi && key.name === 'u' && key.ctrl) {
    self.move(-((self.height - self.iheight) / 2) | 0);
    self.screen.render();
    return;
  }
  if (self.vi && key.name === 'd' && key.ctrl) {
    self.move((self.height - self.iheight) / 2 | 0);
    self.screen.render();
    return;
  }
  if (self.vi && key.name === 'b' && key.ctrl) {
    self.move(-(self.height - self.iheight));
    self.screen.render();
    return;
  }
  if (self.vi && key.name === 'f' && key.ctrl) {
    self.move(self.height - self.iheight);
    self.screen.render();
    return;
  }
  if (self.vi && key.name === 'h' && key.shift) {
    self.move(self.childBase - self.selected);
    self.screen.render();
    return;
  }
  if (self.vi && key.name === 'm' && key.shift) {
  ...
  if (self.vi && key.name === 'l' && key.shift) {
  ...
  if (self.vi && key.name === 'g' && !key.shift) {
    self.select(0);
    self.screen.render();
    return;
  }
